...
// R
static void
set_battery_peripheral_status(struct zmk_widget_screen *widget,
...
ZMK_SUBSCRIPTION(widget_battery_peripheral_status,
...
static int display_activity_event_handler(const zmk_event_t *eh) {
  ...
  case ZMK_ACTIVITY_ACTIVE:
    ...
    // No need to force a redraw, it will happen automatically if really coming
    // back from sleep (ACTIVE also comes after IDLE)
...
int zmk_widget_screen_init(struct zmk_widget_screen *widget, lv_obj_t *parent) {
  ...
  lv_canvas_set_buffer(top, widget->cbuf, SCREEN_WIDTH, SCREEN_HEIGHT,
                       CANVAS_COLOR_FORMAT);
